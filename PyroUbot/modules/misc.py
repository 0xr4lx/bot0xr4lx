    ...
    if status and hasattr(status, "text"):
        ...
        if pjg <= 100:
            if client.me.is_premium:
                text = f"""
<blockquote><b>? {pong} s????s ???? ??????? : ????</b>
<b>? {tion} ????? ????? : ???? ???? ????? ??????s?</b>
<b>? {yubot} ???? : {bot.me.mention}</b></blockquote>
...
async def carbon_func(client, message):
    ...
    ex = await message.reply("mempro?e?...")
    ...
    await asyncio.gather(
        ...
        client.send_photo(
            message.chat.id,
            carbon,
            caption=f"carboni?ed by :{client.me.mention}",
        ),
    )
    carbon.close()
